...
class TS_lootListener: BaseCampaignEventListener(false) {
    override fun reportEncounterLootGenerated(plugin: FleetEncounterContextPlugin?, loot: CargoAPI?) {
        ...
        if (plugin.loserData.fleet.faction.id == Factions.OMEGA && plugin.loserData.ownCasualties.any { it.member.hullSpec.builtInMods.contains("TS_triadShieldsHullmod") }) {
            loot.addHullmods("TS_triadShieldsHullmod", 1)
        }
    }
}