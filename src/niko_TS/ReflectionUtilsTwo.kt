...
**/
internal object ReflectionUtilsTwo {
    ...
    private val getMethodParametersHandle = MethodHandles.lookup().findVirtual(methodClass, "getParameterTypes", MethodType.methodType(
    ...
    //Cache
    private var fieldsCache = kotlin.collections.HashMap<ReflectedFieldKey, ReflectedField>()
    private var fieldNameCache = kotlin.collections.HashMap<Class<*>, HashSet<String>>()
    private var fieldTypesCache = kotlin.collections.HashMap<Class<*>, HashSet<Class<*>>>()

    private var methodsCache = kotlin.collections.HashMap<ReflectedMethodKey, ReflectedMethod>()
    private var methodNameCache = kotlin.collections.HashMap<Class<*>, HashSet<String>>()
    ...
    @JvmStatic
    fun invoke(methodName: String? = null, instance: Any, vararg arguments: Any?, returnType: Class<*>? = null, parameterCount: Int? = null, originClass: Class<out Any> = instance::class.java, ignoreParams: Boolean = false) : Any?
    {
        val clazz = originClass
        val args = arguments.map { if (it == null) null else it::class.javaPrimitiveType ?: it::class.java }

        var method: Any? = null

        return getMethod(methodName, clazz, returnType, parameterCount, args, ignoreParams = ignoreParams)!!.invoke(instance, *arguments)
    }
    ...
    fun hasVariableOfName(name: String, instance: Any) : Boolean {
        ...
        return fieldNameCache.getOrPut(clazz) {
            ...
            var set = kotlin.collections.HashSet<String>()
    ...
    fun hasVariableOfType(type: Class<*>, instance: Any) : Boolean {
        ...
        return fieldTypesCache.getOrPut(clazz) {
            //Class has not been cached yet, save all method names to the cache
            var set = kotlin.collections.HashSet<Class<*>>()
            val instancesOfFields: Array<out Any> = instance.javaClass.declaredFields as Array<out Any>
            for (field in instancesOfFields) {
    ...
    fun getMethod(methodName: String? = null, clazz: Class<*>, returnType: Class<*>?=null, parameterCount: Int?=null, parameters: List<Class<*>?>?, ignoreParams: Boolean = false) : ReflectedMethod? {
        ...
        return methodsCache.getOrPut(key) {
            ...
            for (method in methods) {
                ...
                if (!ignoreParams && parameters?.isNotEmpty() == true || parameterCount != null) {
