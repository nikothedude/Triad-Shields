...
import niko_TS.subsystems.TS_triadShieldSubsystem
...
class TS_triadShieldsHmod: BaseHullMod() {
    ...
    override fun applyEffectsAfterShipCreation(ship: ShipAPI?, id: String?) {
        ...
        ship.addSubsystem(TS_triadShieldSubsystem(ship))
        return
    }
    ...
    override fun addPostDescriptionSection(
        ...
        for (size in HullSize.entries) {
            val str = TS_triadShields.sizesToStrength[size] ?: continue
            val regen = TS_triadShields.sizesToDiss[size] ?: continue
        ...
        if (!isForModSpec && !Global.getSettings().isShowingCodex) {
            if (ship != null) {
                val strength = TS_triadShields.sizesToStrength[hullSize] ?: 10f
                val diss = TS_triadShields.sizesToDiss[hullSize] ?: 10f
                tooltip.setBulletedListMode(BaseIntelPlugin.BULLET)
                tooltip.addPara(
    ...
    override fun addSModEffectSection(
        tooltip: TooltipMakerAPI?,
        hullSize: HullSize?,
        ship: ShipAPI?,
        width: Float,
        isForModSpec: Boolean,
        isForBuildInList: Boolean
    ) {
        if (tooltip == null) return

        tooltip.addPara(
            "Segments no longer receive enhanced regeneration when lowered, and segment damage taken is increased by %s.",
            5f,
            Misc.getNegativeHighlightColor(),
            "${SMOD_DAMAGE_TAKEN_MULT}x"
        )
    }
}